...
function MySafeAreaConsumer() {
    ...
    const [lastClickedTarget, setLastClickedTarget] = React.useState<React.RefObject<View> | null>(
        null,
    );
    ...
    const onTargetPress = (
        ...
        if (whichTarget === "safeArea") {
            log(
                `[onTargetPress] ${whichTarget} – suppressing display of popup for this particular target.`,
            );
            return;
        }
        ...
        target.measureInWindow((x: number, y: number, width: number, height: number) => {
            log(
                `[target.onTargetPress] measureInWindow:\n- sourceView: ${JSON.stringify({
                    x,
                    y,
                    width,
                    height,
    ...
    const onTargetLayout = (e: LayoutChangeEvent, targetRef: React.RefObject<View>) => {
        ...
        target.measureInWindow((x: number, y: number, width: number, height: number) => {
            log(
                `[target.onLayout] measureInWindow:\n- sourceView: ${JSON.stringify({
                    x,
                    y,
                    width,
                    height,
    ...
    return (
        <View style={styles.container}>
            <TouchableWithoutFeedback
                onPress={event => onTargetPress(event, unsafeTopRef, "unsafeTop")}
            >
                <View
                    ref={unsafeTopRef}
                    onLayout={e => onTargetLayout(e, unsafeTopRef)}
                    style={[
            ...
            <View style={styles.centralRow}>
                <TouchableWithoutFeedback
                    onPress={event => onTargetPress(event, unsafeLeftRef, "unsafeLeft")}
                >
                    <View
                        ref={unsafeLeftRef}
                        onLayout={e => onTargetLayout(e, unsafeLeftRef)}
                        style={[
                ...
                <TouchableWithoutFeedback
                    onPress={event => onTargetPress(event, safeAreaRef, "safeArea")}
                >
                    <View
                        ref={safeAreaRef}
                        style={[
                    ...
                    >
                        ...
                        >
                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, nwTargetRef, "nw")}
                            >
                                <View
                                    ref={nwTargetRef}
                                    onLayout={e => onTargetLayout(e, nwTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, nTargetRef, "n")}
                            >
                                <View
                                    ref={nTargetRef}
                                    onLayout={e => onTargetLayout(e, nTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, neTargetRef, "ne")}
                            >
                                <View
                                    ref={neTargetRef}
                                    onLayout={e => onTargetLayout(e, neTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>
                        </View>
                        ...
                        >
                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, wTargetRef, "w")}
                            >
                                <View
                                    ref={wTargetRef}
                                    onLayout={e => onTargetLayout(e, wTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, centralTargetRef, "central")}
                            >
                                <View
                                    ref={centralTargetRef}
                                    onLayout={e => onTargetLayout(e, centralTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, eTargetRef, "e")}
                            >
                                <View
                                    ref={eTargetRef}
                                    onLayout={e => onTargetLayout(e, eTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>
                        </View>
                        ...
                        >
                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, swTargetRef, "sw")}
                            >
                                <View
                                    ref={swTargetRef}
                                    onLayout={e => onTargetLayout(e, swTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, sTargetRef, "s")}
                            >
                                <View
                                    ref={sTargetRef}
                                    onLayout={e => onTargetLayout(e, sTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>

                            <TouchableWithoutFeedback
                                onPress={event => onTargetPress(event, seTargetRef, "se")}
                            >
                                <View
                                    ref={seTargetRef}
                                    onLayout={e => onTargetLayout(e, seTargetRef)}
                                    style={[styles.target]}
                                ></View>
                            </TouchableWithoutFeedback>
                        </View>
                ...
                <TouchableWithoutFeedback
                    onPress={event => onTargetPress(event, unsafeRightRef, "unsafeRight")}
                >
                    <View
                        ref={unsafeRightRef}
                        onLayout={e => onTargetLayout(e, unsafeRightRef)}
                        style={[
                            styles.unsafeHorizontal,
                            {
                                width: insets.right,
            ...
            <TouchableWithoutFeedback
                onPress={event => onTargetPress(event, unsafeBottomRef, "unsafeBottom")}
            >
                <View
                    ref={unsafeBottomRef}
                    onLayout={e => onTargetLayout(e, unsafeBottomRef)}
                    style={[
                        styles.unsafeVertical,
                        {
